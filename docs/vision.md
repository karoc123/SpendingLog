...
* **Thumb-Friendly Entry:** The entry form is optimized for one-handed use: date/category/notes at the top (rarely changed), amount and description at the bottom for easy thumb access.
...
* **Next Transaction Preview:** When editing a recurring rule, I can see the calculated next transaction date to verify the schedule is correct. Inactive rules show the date greyed out.
...
* **Multiple Import Formats:** I can import data from different sources:
  ...
  - **DKB Bank CSV:** German bank export format. The app automatically looks up categories by recipient name from my transaction history, so common purchases (groceries, gas) are auto-categorized.
